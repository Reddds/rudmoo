...
/**
 * The pronounce question type.
 *
 * @copyright  2013 Gordon Bateson (gordon.bateson@gmail.com)
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_pronounce extends question_type {
    ...
    public function find_standard_scripts() {
        ...
        if ($version) {
            ...
            $PAGE->requires->jquery_plugin('ui.touch-punch', 'qtype_pronounce');
        } else {
            // Moodle <= 2.6.
...
//        if ($countanswers < 2) {
//            $result->notice = get_string('notenoughanswers', 'qtype_pronounce', '2');
//            return $result;
//        }

        $question->feedback = range(1, 5);//$countanswers
...
//            'selecttype' => $question->selecttype,
        ...
        // Add/update $options for this pronounce question.
        if ($options->id = $DB->get_field('qtype_pronounce_options', 'id', array('questionid' => $question->id))) {
            if (! $DB->update_record('qtype_pronounce_options', $options)) {
                $result->error = get_string('cannotupdaterecord', 'error', 'qtype_pronounce_options (id='.$options->id.')');
                return $result;
            }
        } else {
            unset($options->id);
            if (! $options->id = $DB->insert_record('qtype_pronounce_options', $options)) {
                $result->error = get_string('cannotinsertrecord', 'error', 'qtype_pronounce_options');
                return $result;
            }
        }
...
//    }
    ...
    public function get_question_options($question) {
        ...
        // Load the options.
        if (!$question->options = $DB->get_record('qtype_pronounce_options', array('questionid' => $question->id))) {
...
//            echo $OUTPUT->notification('Error: Missing question answers for pronounce question ' . $question->id . '!');
//            return false;
//        }
    ...
    public function delete_question($questionid, $contextid) {
        global $DB;
        $DB->delete_records('qtype_pronounce_options', array('questionid' => $questionid));
    ...
    public function import_from_gift($lines, $question, $format, $extra=null) {
        global $CFG;
        require_once($CFG->dirroot.'/question/type/pronounce/question.php');
        ...
        $search = '/^(' . $questionname . ')\s*\{>\s*(' . $phraseid . ')\s*\s*(' . $recordattemptscount . ')\s*(' . $selecttype . ')\s*' .
        ...
        $recordattemptscount = trim($matches[2]);
        ...
        // Set "recordattemptscount" field from $recordattemptscount.
        if (is_numeric($recordattemptscount) && $recordattemptscount > 2 && $recordattemptscount <= count($lines)) {
            $recordattemptscount = intval($recordattemptscount);
        } else {
            $recordattemptscount = min(6, count($lines));
        }
        $this->set_layout_select_count_grading($question, $layouttype, $selecttype, $recordattemptscount, $gradingtype, $showgrading);
    ...
    /**
     * Given question object, returns array with array layouttype, selecttype, recordattemptscount, gradingtype, showgrading
     * where layouttype, selecttype, gradingtype and showgrading are string representations.
     *
     * @param object $question
     * @return array(layouttype, selecttype, recordattemptscount, gradingtype)
     */
    public function extract_layout_select_count_grading($question) {

        switch ($question->options->layouttype) {
            case qtype_pronounce_question::LAYOUT_VERTICAL:
                $layout = 'VERTICAL';
                break;
            case qtype_pronounce_question::LAYOUT_HORIZONTAL:
                $layout = 'HORIZONTAL';
                break;
            default:
        ...
        switch ($question->options->selecttype) {
            case qtype_pronounce_question::SELECT_ALL:
                $select = 'ALL';
                break;
            case qtype_pronounce_question::SELECT_RANDOM:
                $select = 'RANDOM';
                break;
            case qtype_pronounce_question::SELECT_CONTIGUOUS:
                $select = 'CONTIGUOUS';
                break;
            default:
        ...
        $count = $question->options->recordattemptscount;
    ...
    public function export_to_gift($question, $format, $extra=null) {
        ...
        list($layouttype, $selecttype, $phraseid, $recordattemptscount, $gradingtype, $showgrading) = $this->extract_layout_select_count_grading($question);
        $output = $question->questiontext.'{>'.$phraseid.' '.
                                               $recordattemptscount.' '.
                                               $selecttype.' '.
                                               $layouttype.' '.
                                               $gradingtype.' '.
    ...
    public function export_to_xml($question, qformat_xml $format, $extra=null) {
        ...
        $output = '';
        $output .= "    <phraseid>$phraseid</phraseid>\n";
        $output .= "    <layouttype>$layouttype</layouttype>\n";
        $output .= "    <selecttype>$selecttype</selecttype>\n";
        $output .= "    <recordattemptscount>$recordattemptscount</recordattemptscount>\n";
     ...
     *
     * @param array $data
     * @param qtype_pronounce $question (or null)
     * @param qformat_xml $format
     * @param string $extra (optional, default=null)
    ...
    public function fix_questionname($name, $defaultname='', $maxnamelength = 42) {
        if (trim($name) == '') {
            if ($defaultname) {
                ...
                $name = get_string('defaultquestionname', 'qtype_pronounce');
            }
        }
    ...
    /**
     * Set layouttype, selecttype, recordattemptscount, gradingtype, showgrading based on their textual representation
     *
    ...
    public function set_layout_select_count_grading(&$question, $layout, $select, $count, $grading, $show) {
        ...
        // Set default values.
        $layouttype  = qtype_pronounce_question::LAYOUT_VERTICAL;
        $selecttype  = qtype_pronounce_question::SELECT_RANDOM;
        $recordattemptscount = 3;
        $gradingtype = qtype_pronounce_question::GRADING_RELATIVE_NEXT_EXCLUDE_LAST;
        $showgrading = 1;
        ...
        switch (strtoupper($layout)) {
            ...
            case 'H':
            case '1':
                $layouttype = qtype_pronounce_question::LAYOUT_HORIZONTAL;
                break;
...
//            case 'ALL':
//            case 'EXACT':
//                $selecttype = qtype_pronounce_question::SELECT_ALL;
//                break;
//            case 'RANDOM':
//            case 'REL':
//                $selecttype = qtype_pronounce_question::SELECT_RANDOM;
...
//            case 'CONTIGUOUS':
//            case 'CONTIG':
//                $selecttype = qtype_pronounce_question::SELECT_CONTIGUOUS;
//                break;
//        }

        // Set "recordattemptscount" from $count
        // this used to be ($count - 2).
        if (is_numeric($count)) {
            $recordattemptscount = intval($count);
        }

        // Set "gradingtype" from $grading.
        switch (strtoupper($grading)) {
            case 'ALL_OR_NOTHING':
                $gradingtype = qtype_pronounce_question::GRADING_ALL_OR_NOTHING;
                break;
            case 'ABS':
            case 'ABSOLUTE':
            case 'ABSOLUTE_POSITION':
                $gradingtype = qtype_pronounce_question::GRADING_ABSOLUTE_POSITION;
                break;
            case 'REL':
            case 'RELATIVE':
            ...
            case 'RELATIVE_ONE_PREVIOUS_AND_NEXT':
                $gradingtype = qtype_pronounce_question::GRADING_RELATIVE_ONE_PREVIOUS_AND_NEXT;
                break;
            ...
            case 'LONGEST_ORDERED_SUBSET':
                $gradingtype = qtype_pronounce_question::GRADING_LONGEST_ORDERED_SUBSET;
                break;
            case 'LONGEST_CONTIGUOUS_SUBSET':
                $gradingtype = qtype_pronounce_question::GRADING_LONGEST_CONTIGUOUS_SUBSET;
                break;
        }
        ...
        $question->phraseid  = $phraseid;
        $question->recordattemptscount = $recordattemptscount;
